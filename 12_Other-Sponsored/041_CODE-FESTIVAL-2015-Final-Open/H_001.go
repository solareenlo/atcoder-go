...
func main() {
	...
	X := NewSegTree(m+1, op, e)
	Y := NewSegTree(m+1, op, e)
	X.Set(0, 0)
	Y.Set(0, 0)
	for _, i := range P {
		s := i.x
		l := i.y
		t := s + l
		z := op(X.Prod(0, s)+l, Y.Prod(s, t)+s+t)
		X.Set(t, op(X.Get(t), z))
		Y.Set(t, op(Y.Get(t), z-2*t))
	}
	fmt.Println(X.AllProd())
}

func e() int          { return -1 << 30 }
