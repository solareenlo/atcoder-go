...
func main() {
	...
	for i := 1; i <= N; i++ {
		...
		copy(fn, dpn[i-Len:i])
		fsum := make([]int, Len)
		copy(fsum, dpsum[i-Len:i])
		tmp := min(Len*2-1, len(trans))
		g := make([]int, tmp)
		copy(g, trans[:tmp])
		convn := con.Convolve(g, fn)
		convsum := con.Convolve(g, fsum)
		for j := 0; j < Len; j++ {
			if i+j >= len(dpn) {
				break
			}
			if j+Len-1 >= len(convn) {
				break
			}
			dpn[i+j] = (dpn[i+j] + convn[j+Len-1]) % MOD
			dpsum[i+j] = (((dpsum[i+j] + convsum[j+Len-1]) % MOD) + convn[j+Len-1]*cost%MOD) % MOD
		}
	}
	fmt.Println(dpsum[N])
}
