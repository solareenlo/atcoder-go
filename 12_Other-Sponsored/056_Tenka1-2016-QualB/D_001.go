...
func main() {
	...
	tmp := make([]int, a+1)
	seg := NewLazySegTree(tmp, op, e, f, f, id)
	for i := 0; i < n; i++ {
		for _, t := range C[i] {
			seg.RangeApply(t+1, a+1, X[t])
		}
		for _, t := range D[i] {
			seg.RangeApply(t+1, a+1, -X[t])
		}
		for _, v := range V[i] {
			Z[v] = seg.Prod(S[v], T[v]) + A[i]
		}
	}
	for i := 0; i < b; i++ {
		fmt.Fprintln(out, Z[i])
	}
}
