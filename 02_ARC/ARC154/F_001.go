...
func main() {
	...
	inv[mx] = invMod(fac[mx])
	for i := mx; i > 0; i-- {
		inv[i-1] = inv[i] * i % MOD
	}

	var dfs1 func(int, int) []int
	dfs1 = func(L, R int) []int {
		if R-L == 1 {
			return []int{N, -L}
		}
		mid := (L + R) / 2
		return mul(dfs1(L, mid), dfs1(mid, R))
	}
	A := dfs1(0, N)
	...
	dfs = func(L, R int) ([]int, []int) {
		if R-L == 1 {
			return []int{A[L]}, []int{1, -L}
		}
		mid := (L + R) / 2
		p1, q1 := dfs(L, mid)
		p2, q2 := dfs(mid, R)
		return plus(mul(p1, q2), mul(p2, q1)), mul(q1, q2)
	}
	p, q := dfs(0, N+1)
	resize(&q, M+1)
	q = inv_fps(q)
	...
	resize(&F, M+1)
	for i := 1; i <= M; i++ {
		fmt.Fprintln(out, (F[i]*fac[N]%MOD)*fac[i]%MOD)
	}
}
