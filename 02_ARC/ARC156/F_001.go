...
func main() {
	...
	g := NewMfgraph(n*2 + M*2 + 2)
	for i := 1; i <= n; i++ {
		if vis[a[i]] {
			g.AddEdge(S, i, 1)
			g.AddEdge(i, n*2+a[i], 1)
			g.AddEdge(i, n*2+b[i], 1)
			g.AddEdge(n*2+M+a[i], n+i, 1)
			g.AddEdge(n*2+M+c[i], n+i, 1)
			g.AddEdge(n+i, T, 1)
		} else {
			vis[a[i]] = true
			g.AddEdge(i, S, 1)
			g.AddEdge(n*2+a[i], i, 1)
			g.AddEdge(i, n*2+b[i], 1)
			g.AddEdge(n+i, n*2+M+a[i], 1)
			g.AddEdge(n*2+M+c[i], n+i, 1)
			g.AddEdge(T, n+i, 1)
		}
	}
	for i := 1; i <= M; i++ {
		if vis[i] {
			id[i] = g.AddEdge(n*2+M+i, n*2+i, 1)
		} else {
			id[i] = g.AddEdge(n*2+i, n*2+M+i, 1)
		}
	}
	g.Flow(S, T)
	tp := 0
	for i := 1; i <= M; i++ {
		tmp := g.GetEdge(id[i]).flow
		if (tmp == 0 && vis[i]) || (tmp != 0 && !vis[i]) {
			tp++
			st[tp] = i
		}
	}
