...
func main() {
	...
	for i := 1; i <= n; i++ {
		...
		g.AddEdge(S, id[i][0], inf)
		g.AddEdge(id[i][100], T, inf)
		if a[i] < b[i] {
			for j := 0; j < 100; j++ {
				g.AddEdge(id[i][j], id[i][j+1], max(100-j-a[i], 0))
				g.AddEdge(id[i][j+1], id[i][j], inf)
			}
		} else {
			for j := 0; j < 100; j++ {
				g.AddEdge(id[i][j], id[i][j+1], max(j+1-a[i], 0))
				g.AddEdge(id[i][j+1], id[i][j], inf)
			}
		}
	}
	for i := 1; i <= m; i++ {
		...
		g.AddEdge(id[x][101-v], id[y][v-1], inf)
	}
	fmt.Println(sum + g.Flow(S, T))
}
