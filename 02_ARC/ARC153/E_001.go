...
func main() {
	var f [200005]int
	var fc, iv [400005]int

	var s string
	fmt.Scan(&s)
	n := len(s)
	s = " " + s + strings.Repeat(" ", 200005-n)
	fc[0] = 1
	for i := 1; i <= n+n; i++ {
		fc[i] = fc[i-1] * i % MOD
	}
	iv[n+n] = invMod(fc[n+n])
	for i := n + n; i > 0; i-- {
		iv[i-1] = iv[i] * i % MOD
	}
	var p int
	for p = 1; p <= n && s[p] <= s[p+1]; p++ {

	}
	for r, l := p, 0; r > 0; r = l - 1 {
		for l = r - 1; l != 0 && s[l] == s[r]; l-- {
		...
		for i := 0; i <= n; i++ {
			b = append(b, (fc[r-l-1+i]*iv[i]%MOD)*iv[r-l-1]%MOD)
		}
		a = con.Convolve(a, b)
		for i := r + 1; i < q; i++ {
			f[i] = a[i-r]
