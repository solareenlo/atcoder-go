...
func main() {
	...
	card = make([]pair, n+1)
	for i := 1; i <= n; i++ {
		fmt.Fscan(in, &card[i].x, &card[i].y)
	}
	tmp := card[1:]
	sort.Slice(tmp, func(a, b int) bool {
		return (tmp[a].x*tmp[b].y > tmp[b].x*tmp[a].y)
	})
	fin := get(1, n)
	u := 0
	for i := 0; i < n+1; i++ {
...
func get(l, r int) result {
	if l == r {
		var ok result
		ok[0] = []int{0, (card[l].y * InvMod(2*card[l].y-card[l].x)) % MOD}
		ok[1] = []int{((card[l].y - card[l].x) * InvMod(2*card[l].y-card[l].x)) % MOD, 0}
		return ok
	} else {
		var ok result
		mid := (l + r) / 2
		lt := get(l, mid)
		rt := get(mid+1, r)
		a := con.Convolve(lt[0], rt[0])
		b := con.Convolve(lt[0], rt[1])
		rt[0] = reverseOrderInt(rt[0])
		rt[1] = reverseOrderInt(rt[1])
		c := con.Convolve(lt[1], rt[0])
		d := con.Convolve(lt[1], rt[1])
		for i := 0; i <= r-l+1; i++ {
			ok[0] = append(ok[0], (a[i]+d[i])%MOD)
			ok[1] = append(ok[1], (b[i]+c[i])%MOD)
		}
		return ok
	}
}
