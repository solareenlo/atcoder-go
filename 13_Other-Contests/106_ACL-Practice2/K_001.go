...
func main() {
	...
	for i := 0; i < q; i++ {
		...
		if tmp == 0 {
			var c, d int
			fmt.Fscan(in, &c, &d)
			seg.RangeApply(l, r, F{c, d})
		} else {
			fmt.Fprintln(out, seg.Prod(l, r).a)
		}
	}
}

func op(l, r P) P          { return P{(l.a + r.a) % MOD, l.size + r.size} }
func e() P                 { return P{0, 0} }
func mapping(f F, s P) P   { return P{(s.a*f.a%MOD + s.size*f.b%MOD) % MOD, s.size} }
func composition(f, g F) F { return F{f.a * g.a % MOD, (f.a*g.b%MOD + f.b) % MOD} }
func id() F                { return F{1, 0} }

type P struct{ a, size int }
