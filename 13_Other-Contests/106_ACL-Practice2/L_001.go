...
func main() {
	...
	for i := 0; i < q; i++ {
		...
		if t == 1 {
			...
			fmt.Fprintln(out, seg.Prod(l, r).inversion)
		}
	}
}
...
func e() P { return P{0, 0, 0} }

func mapping(l F, r P) P {
	if !l.d {
		return r
	}
	return P{r.one, r.zero, r.one*r.zero - r.inversion}
}

func composition(l, r F) F { return F{(l.d && !r.d) || (!l.d && r.d)} }

func id() F { return F{false} }

type P struct{ zero, one, inversion int }
