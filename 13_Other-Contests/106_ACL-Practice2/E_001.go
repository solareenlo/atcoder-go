...
func main() {
	...
	g := newMinCostFlow(2*n + 2)

	s := 2 * n
	t := 2*n + 1
	g.AddEdge(s, t, n*k, INF)
	...
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			var a int
			fmt.Fscan(in, &a)
			g.AddEdge(i, n+j, 1, INF-a)
		}
	}
	res := g.FlowL(s, t, n*k)
	fmt.Println(n*k*INF - res[1])

	grid := make([][]string, n)
	for i := range grid {
		grid[i] = strings.Split(strings.Repeat(".", n), "")
	}
	...
	for i := 0; i < n; i++ {
		fmt.Fprintln(out, strings.Join(grid[i], ""))
	}
}
