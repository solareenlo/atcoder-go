...
func main() {
	...
	ig[NN*2-1] = int(mint(gg[NN*2-1]).inv())
	for i := NN*2 - 2; i >= 0; i-- {
		ig[i] = ig[i+1] * (i + 1) % MOD
	}
	...
	ans, b := 0, 0
	for i := 0; i < n; i++ {
		b = ((b*i%MOD)*2%MOD + ff[i]) % MOD
		ans = (ans + b*t%MOD) % MOD
		t = t * (gg[i*2+2] * ig[i*2+3] % MOD) % MOD
	}

	fmt.Println(ans * gg[n] % MOD)
}
...
func rec(l, r int) {
	...
	low := l + mid
	hi := mid + r - 1
	x := make([]int, len(ff[l:mid]))
	copy(x, ff[l:mid])
	y := make([]int, len(gg[low:hi]))
	copy(y, gg[low:hi])
	for i := l; i < mid; i++ {
		x[i-l] = x[i-l] * ig[i*2] % MOD
	}
	y = ReverseOrderInt(y)
	add := con.Convolve(x, y)
	for i := mid; i < r; i++ {
		ff[i] = (ff[i] - add[r-l+mid-i-2] + MOD) % MOD
	}
