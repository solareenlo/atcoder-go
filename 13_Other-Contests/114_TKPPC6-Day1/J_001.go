...
func main() {
	...
	g := NewMinCostFlow(2 * N * N)
	for i := 0; i < N; i++ {
		for j := 0; j < N; j++ {
			var A int
			fmt.Fscan(in, &A)
			g.AddEdge(N*i+j, N*i+j+N*N, 1, INF-A)
			g.AddEdge(N*i+j, N*i+j+N*N, 1, INF)
			if i+1 < N {
				g.AddEdge(N*i+j+N*N, N*(i+1)+j, 2, 0)
			}
			if j+1 < N {
				g.AddEdge(N*i+j+N*N, N*i+j+1, 2, 0)
			}
		}
	}
	fmt.Println(INF*(4*N-2) - g.FlowL(0, 2*N*N-1, 2)[1])
}
