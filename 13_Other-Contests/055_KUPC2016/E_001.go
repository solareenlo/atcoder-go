...
func main() {
	...
	for u := 0; u < H*W; u++ {
		...
		graph.AddEdge(2*u, 2*u+1, cost)
		if A[u] {
			graph.AddEdge(S, 2*u+1, (1 << 30))
		}
		if u%W != 0 {
			v := u - 1
			graph.AddEdge(2*u+1, 2*v, (1 << 30))
			graph.AddEdge(2*v+1, 2*u, (1 << 30))
		}
		if u/W != 0 {
			v := u - W
			graph.AddEdge(2*u+1, 2*v, (1 << 30))
		...
		if u%W == 0 || u%W == W-1 || u/W == 0 || u/W == H-1 {
			graph.AddEdge(2*u+1, T, (1 << 30))
		}
	}
	flow := graph.Flow(S, T)
	if flow > (1 << 28) {
