...
func main() {
	...
	for i := 0; i < q; i++ {
		...
		if t == 0 {
			var v, k int
			fmt.Fscan(IN, &v, &k)
			seg.RangeApply(in[v], in[v]+sz[v], F{k / sz[v], 1})
		} else {
			var v int
			fmt.Fscan(IN, &v)
			ans := 0
			...
			for {
				...
				if nxt[a] != nxt[b] {
					ans += seg.Prod(in[nxt[b]], in[b]+1).x
					seg.RangeApply(in[nxt[b]], in[b]+1, F{0, 0})
					b = par[nxt[b]]
				} else {
					ans += seg.Prod(in[a], in[b]+1).x
					seg.RangeApply(in[a], in[b]+1, F{0, 0})
					break
				}
			}
			fmt.Println(ans)
...
func dfs_sz(u int) {
	sz[u] = 1
	if len(G[u]) >= 2 && G[u][0] == par[u] {
		G[u][0], G[u][1] = G[u][1], G[u][0]
	}
	for v := range G[u] {
		if G[u][v] == par[u] {
			continue
...
func mp(f F, x P) P { return P{x.x*f.y + f.x*x.y, x.y} }
func cmp(f, g F) F  { return F{f.x + f.y*g.x, f.y * g.y} }
func id() F         { return F{0, 1} }
