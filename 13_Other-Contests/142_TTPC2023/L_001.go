...
func main() {
	...
	lcm1 := m[0] * m[1] / gcd(m[0], m[1])
	lcm2 := m[2] * m[3] / gcd(m[2], m[3])

	f1 := make([]int, lcm2)
	f2 := make([]int, lcm2)

	for i := 0; i < lcm1; i++ {
		if s[0][i%m[0]] == 'T' && s[1][i%m[1]] == 'T' {
			f1[(lcm2-i%lcm2)%lcm2] += 1
		}
	}

	for i := 0; i < lcm2; i++ {
		if s[2][i%m[2]] == 'P' && s[3][i%m[3]] == 'C' {
			f2[i] += 1
		}
	}

	fmint := con.Convolve(f1, f2)
	fll := make([]int, lcm2)

	for i := 0; i < len(fmint); i++ {
		fll[i%lcm2] = (fll[i%lcm2] + fmint[i]) % MOD
	}

	fllsum := 0

	for i := 0; i < lcm2; i++ {
		fllsum = (fllsum + fll[lcm1*i%lcm2]) % MOD
	}
