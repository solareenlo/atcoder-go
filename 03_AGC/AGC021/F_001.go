...
func main() {
	...
	for j := 1; j <= m; j++ {
		...
		y := make([]int, n+3)
		for i := 0; i <= n+2; i++ {
			x[i] = dp[i][j-1] * invf[i] % mod
			y[i] = invf[i]
		}
		z := con.Convolve(x, y)
		for i := 0; i <= n; i++ {
			dp[i][j] = (nCrMod(i, 2)+i+1)*dp[i][j-1]%mod + (((z[i+2]-(x[i]*y[2]%mod)+mod)%mod-(x[i+1]*y[1]%mod)+mod)%mod-(x[i+2]*y[0]%mod)+mod)%mod*fact[i+2]%mod
			dp[i][j] %= mod
		}
	}
