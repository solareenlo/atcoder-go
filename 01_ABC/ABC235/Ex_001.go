...
func main() {
	...
	for i := 0; i < m; i++ {
		var u, v, w int
		fmt.Fscan(in, &u, &v, &w)
		e = append(e, tuple{w, pair{u, v}})
	}
	sort.Slice(e, func(i, j int) bool {
		return e[i].w < e[j].w || (e[i].w == e[j].w && e[i].p.x < e[j].p.x) || (e[i].w == e[j].w && e[i].p.x == e[j].p.x && e[i].p.y < e[j].p.y)
	})
