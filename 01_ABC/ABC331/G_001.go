...
func solve(C []int) int {
	...
	for i := 1; i <= N; i++ {
		if cnt[i] > 0 {
			...
			v := make([]int, d+1)
			for j := 0; j <= cnt[i]; j++ {
				val := (factorial[cnt[i]] * invfact[j] % MOD) * invfact[cnt[i]-j] % MOD
				if (j & 1) != 0 {
					v[i*j] = (MOD - val) % MOD
				} else {
					v[i*j] = val
	...
	for i := 0; i < len(vecs); i++ {
		heap.Push(pq, pair{-(len(vecs[i])), i})
	}
	for pq.Len() > 1 {
		v1 := vecs[heap.Pop(pq).(pair).y]
		v2 := vecs[heap.Pop(pq).(pair).y]
		v3 := con.Convolve(v1, v2)
		heap.Push(pq, pair{-len(v3), len(vecs)})
		vecs = append(vecs, v3)
	}
	dp := make([]int, N+1)
	for i := 0; i <= N; i++ {
		dp[i] = (MOD - vecs[(*pq)[0].y][i]) % MOD
	}
	ans := 0
	for s := 1; s <= N; s++ {
		ans = (ans + (dp[s]*factorial[s-1]%MOD)*invfact[s]%MOD) % MOD
	}
	ans = ans * N % MOD
	return ans
}
...
var con = NewConvolution(MOD, primitive_root(MOD))
