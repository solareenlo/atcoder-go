...
func main() {
	...
	y := (c - a) / 2
	for i := 0; i < t+1; i++ {
		P[i] = (I[i] * facinv(i+x) % MOD) * facinv(i+y) % MOD
		Q[i] = (I[i] * facinv(i-x) % MOD) * facinv(i-y) % MOD
		R[i] = (I[i] * I[i] % MOD) * I[i] % MOD
	}
	S := con.Convolve(P, Q)
	T := con.Convolve(R, R)
	resize(&S, t+1)
	resize(&T, t+1)
	for i := 0; i < t+1; i++ {
		S[i] = S[i] * ((F[i] * F[i] % MOD) * F[i] % MOD) % MOD
		T[i] = T[i] * ((F[i] * F[i] % MOD) * F[i] % MOD) % MOD
	}
	ans := con.Convolve(S, polyinv(T, t+1))[t] * PowMod(invMod(2), 3*t) % MOD
	fmt.Println(ans)
}
