...
func main() {
	...
	v := make([]int, 0)
	cur := A % mod
	for i, j := 2, A-1; i <= n; i, j = i+1, j-1 {
		cur = divMod((cur*j)%mod, i)
		v = append(v, cur)
	}
	sol(2, n, v)
}

func sol(l, r int, v []int) []int {
	if l == r {
		if l == n {
			fmt.Printf("%d\n", v[0])
			os.Exit(0)
	...
	m := (l + r) >> 1
	a := sol(l, m, ga(0, m-l+1, v))
	return con.Convolve(a, sol(m+1, r, ga(m-l+1, len(v), con.Convolve(v, a))))
}

func ga(l, r int, v []int) []int {
	res := make([]int, 0)
	for i := l; i < r; i++ {
		res = append(res, v[i])
	}
