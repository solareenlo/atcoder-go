...
func main() {
	...
	g = NewTwosat(N)
	for i := 1; i <= n; i++ {
		g.AddClause(ord(i, m), true, ord(i, m), true)
		for j := 0; j < m; j++ {
			g.AddClause(ord(i, j+1), true, ord(i, j), false)
		}
	}
	for q > 0 {
		q--
...
func work(a, b, l, r int) {
	for i := 0; i <= m; i++ {
		if i >= r {
			g.AddClause(ord(a, i), true, ord(a, i), true)
		} else if r-i <= m {
			g.AddClause(ord(a, i), true, ord(b, r-i-1), true)
		}
		if i+m < l {
			g.AddClause(ord(a, i), false, ord(a, i), false)
		} else if i < l {
			g.AddClause(ord(a, i), false, ord(b, l-i-1), false)
		}
	}
}

func ord(i, j int) int {
	return (m+1)*(i-1) + j
}
