...
func dc(L, R int) {
	if L+1 == R {
		n := L
		if n >= 2 {
			f[n] = divMod((f[n]+j[n-1])%mod, (n - 1))
		}
		if n >= 1 {
			fg := ((f[n] + g[n]) % mod) * n % mod
			for i := n; i < B; i += n {
				j[i] += fg
				j[i] %= mod
			}
	...
	dc(L, M)
	if L == 0 {
		fj := con.Convolve(get(f, 0, M), get(j, 0, M))
		for i := M; i < min(R, len(fj)); i++ {
			f[i] += fj[i]
			f[i] %= mod
		}
	} else {
		fj1 := con.Convolve(get(f, L, M), get(j, 0, R-L))
		fj2 := con.Convolve(get(f, 0, R-L), get(j, L, M))
		for i := M; i < R; i++ {
			f[i] += fj1[i-L] + fj2[i-L]%mod
			f[i] %= mod
		}
	}
	dc(M, R)
