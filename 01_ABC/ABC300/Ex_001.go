...
func Bostan_Mori(a, c []int, n int) int {
	...
	for n != 0 {
		cinv := make([]int, len(c))
		copy(cinv, c)
		for i := 1; i < len(cinv); i += 2 {
			cinv[i] = (MOD - cinv[i]) % MOD
		}
		a = con.Convolve(a, cinv)
		c = con.Convolve(c, cinv)
		na := make([]int, 0)
		if n%2 == 1 {
			if len(a)%2 == 1 {
				a = append(a, 0)
			...
			for i := 0; i < len(a); i += 2 {
				na = append(na, (a[i]+a[i+1])%MOD)
			}
		} else {
			for i := (n & 1); i < len(a); i += 2 {
				na = append(na, a[i])
			}
		}
		a, na = na, a
