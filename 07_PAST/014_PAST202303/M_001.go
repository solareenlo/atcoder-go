...
func main() {
	...
	seg := NewSegTree(B, op, e)
	for i := 0; i < N; i++ {
		seg_l_v = A[i]
		idx := seg.MaxRight(0, cmp)
		if idx == M {
			fmt.Println("No")
			fmt.Println(i + 1)
			return
		}
		seg.Set(idx, seg.Get(idx)-A[i])
	}
	fmt.Println("Yes")
	for i := 0; i < M; i++ {
		if i == N-1 {
			fmt.Println(B[i] - seg.Get(i))
		} else {
			fmt.Printf("%d ", B[i]-seg.Get(i))
		}
	}
}
...
func cmp(x int) bool { return x < seg_l_v }
