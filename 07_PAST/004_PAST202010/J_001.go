...
func main() {
	...
	var S string
	fmt.Fscan(in, &N, &M, &Xab, &Xac, &Xbc, &S)

	G := newMinCostFlow(N + 6)

	for i := 0; i < len(S); i++ {
		if S[i] == 'A' {
			G.AddEdge(i, N, 1, 0)
			G.AddEdge(N+1, i, 1, Xab)
			G.AddEdge(i, N+2, 1, 0)
			G.AddEdge(N+3, i, 1, Xac)
		}
		if S[i] == 'B' {
			G.AddEdge(N, i, 1, Xab)
			G.AddEdge(i, N+1, 1, 0)
			G.AddEdge(i, N+4, 1, 0)
			G.AddEdge(N+5, i, 1, Xbc)
		}
		if S[i] == 'C' {
			G.AddEdge(N+2, i, 1, Xac)
			G.AddEdge(i, N+3, 1, 0)
			G.AddEdge(N+4, i, 1, Xbc)
			G.AddEdge(i, N+5, 1, 0)
		}
	}
