...
func main() {
	...
	for l := 0; l < n; l++ {
		var a int
		fmt.Fscan(in, &a)
		V[l+1] = append(V[l+1], pair{l, a})
	}
	for m > 0 {
		m--
		var b, l, r int
		fmt.Fscan(in, &b, &l, &r)
		V[r] = append(V[r], pair{l - 1, b})
	}
	seg := NewSegTree(n+1, op, e)
	seg.Set(0, 0)
	for r := 1; r <= n; r++ {
		for _, key := range V[r] {
			l, a := key.x, key.y
			seg.Set(r, op(seg.Get(r), seg.Prod(l, r)+a))
		}
	}
	fmt.Println(seg.Get(n))
}

func e() int          { return 1 << 60 }
