...
func main() {
	...
	G := newMinCostFlow(len(t))

	for i := 0; i < len(t)-1; i++ {
		G.AddEdge(i, i+1, 1, t[i+1]-t[i])
		G.AddEdge(i+1, i, 1, t[i+1]-t[i])
	}

	for i := 0; i < M; i++ {
		d0 := lowerBound(t, A[i])
		d1 := lowerBound(t, B[i])
		G.AddEdge(d0, d1, 1, C[i])
		G.AddEdge(d1, d0, 1, C[i])
	}

	fmt.Println(G.FlowL(0, len(t)-1, 1)[1])
}
