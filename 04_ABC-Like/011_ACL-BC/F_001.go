...
func main() {
	...
	for i := 0; i < n+n; i++ {
		...
		fmt.Fscan(in, &x)
		h[x]++
		f = append(f, f[len(f)-1]*(i+1)%mod)
		v[0] = append(v[0], v[0][len(v[0])-1]*(i+i+1)%mod)
	}
	...
	for i := 1; i < 111111; i++ {
		...
		t := make([]int, 0)
		for j := 0; j <= h[i]/2; j++ {
			t = append(t, divMod(v[0][j]*f[h[i]]%mod, f[j+j]*f[h[i]-j-j]%mod))
		}
		v = append(v, t)
	}

	for i := 1 - n%2; i < len(v[0]); i += 2 {
		v[0][i] = (v[0][i]*-1 + mod) % mod
	}
	conv(0, len(v))
	fmt.Println(v[0][n])
}
