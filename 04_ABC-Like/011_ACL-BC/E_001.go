...
func op(a, b S) S {
	return S{(a.x + b.x) % MOD, (a.y + b.y) % MOD}
}
...
func mapping(f F, s S) S {
	if f.d != 0 {
		return S{s.x, f.d * s.x % MOD}
	}
	return s
}

func composition(a, b F) F {
	if a.d != 0 {
		return a
	}
	return b
...
func main() {
	...
	u[0] = S{1, 1}
	for i := 1; i < n; i++ {
		u[i].y = u[i-1].x * 10 % MOD
		u[i].x = u[i].y
	}

	seg := newLazySegtree(u, op, e, mapping, composition, id)

	for i := 0; i < q; i++ {
		var l, r, d int
		fmt.Fscan(in, &l, &r, &d)
		seg.RangeApply(n-r, n-l+1, F{d})
		fmt.Fprintln(out, seg.AllProd().y)
	}
}
